...
internal sealed class ReportUxContractScenario : ISmokeTestScenario
{
    ...
    public Task<SmokeTestResult> RunAsync(SmokeTestContext context, CancellationToken cancellationToken = default)
    {
        ...
        RequireContains(rendererSource, "Copy evidence summary", "Report renderer should expose explicit copy buttons for evidence summaries.");
        ...
        RequireContains(rendererSource, "Closed by default", "Raw event shell should label the default folded state.");
        ...
        RequireContains(rendererSource, "No JavaScript required for report navigation", "Report renderer should explain no-JS report navigation.");
        ...
        RequireContains(rendererSource, "Narrative spine summary", "Report renderer should include a copyable narrative-spine summary.");
        RequireContains(rendererSource, "Copy narrative spine", "Report renderer should expose an explicit narrative-spine copy button.");
        RequireContains(rendererSource, "narrative-spine-lead", "Report renderer should style the narrative-spine lead.");
        RequireContains(rendererSource, "process-tree-path", "Report renderer should keep process tree image/path hints compact.");
        RequireContains(rendererSource, "depth ", "Report renderer should expose process tree depth badges.");
        ...
        RequireAnyContains(doc, ["bounded timeline", "按设计保持有界"], "Report UX doc should require bounded timeline rendering.");
        ...
        RequireAnyContains(doc, ["self-noise excluded", "排除 self-noise"], "Report UX doc should document process tree self-noise exclusion.");
        RequireContains(doc, "Registry behavior", "Report UX doc should list registry behavior.");
        RequireAnyContains(doc, ["Right-click", "right-click", "右键"], "Report UX doc should describe right-click copy.");
        RequireAnyContains(doc, ["raw events only", "只展示最终分类前的 raw events"], "Report UX doc should distinguish live raw events from final classification.");
        RequireAnyContains(doc, ["first 75 raw events", "前 75 条 raw events", "前 75 行"], "Report UX doc should require a slim raw event inline limit.");
        RequireAnyContains(doc, ["25-row native pages", "25 行一页的原生分页"], "Report UX doc should require compact bounded raw event pages.");
        RequireContains(doc, "folded technical field counts", "Report UX doc should require raw event fold-count labels.");
        ...
        RequireAnyContains(doc, ["copyable row ranges", "可复制行范围"], "Report UX doc should require copyable row ranges.");
        ...
        RequireAnyContains(doc, ["native HTML/CSS", "原生 HTML/CSS"], "Report UX doc should keep raw event expansion independent of JavaScript.");
        RequireContains(doc, "print/no-JS fallback", "Report UX doc should require a print and no-JS fallback.");
        RequireContains(doc, "@media print", "Report UX doc should require print-specific fallback layout.");
        ...
        RequireAnyContains(doc, ["must not be used as `href`", "绝对路径不得用于 `href`"], "Report UX doc should forbid absolute local paths as href values.");
        ...
        RequireAnyContains(doc, ["Chinese and English", "中文和英文", "中英"], "Report UX doc should require Chinese and English report rendering support.");
        RequireAnyContains(doc, ["bilingual entry bar", "双语入口栏"], "Report UX doc should require stable in-report bilingual entry links.");
        ...
        RequireAnyContains(doc, ["Cover / 封面", "封面 / Cover"], "Report UX doc should list the cover section.");
        RequireAnyContains(doc, ["Table of contents / 目录", "目录 / Table of contents"], "Report UX doc should list the table of contents section.");
        RequireAnyContains(doc, ["Behavior detections / 行为命中", "行为命中 / Behavior detections"], "Report UX doc should list behavior hits.");
        RequireAnyContains(doc, ["R0 / driver events", "R0 and driver events", "R0 与驱动事件"], "Report UX doc should list R0 driver events.");
        RequireAnyContains(doc, ["Raw normalized events / 原始事件", "原始标准化事件 / Raw normalized events"], "Report UX doc should list raw events.");
    ...
    private static void RequireRenderedReportContract()
    {
        ...
        RequireContains(englishHtml, $"Raw event page 1/3: rows 1-25 of {totalRawEvents}", "Rendered raw event panel should show the first native page.");
        RequireContains(englishHtml, $"Raw event page 3/3: rows 51-75 of {totalRawEvents}", "Rendered raw event panel should show the final inline page.");
        ...
        RequireContains(englishHtml, "Closed by default", "Rendered raw event shell should label the default folded state.");
        RequireContains(englishHtml, "folded technical fields", "Rendered raw page summaries should include folded technical field counts.");
        RequireContains(englishHtml, "Copy raw index summary", "Rendered raw event index cards should include copy buttons.");
        RequireContains(englishHtml, "Copy distribution summary", "Rendered raw event distribution cards should include copy buttons.");
        ...
        RequireContains(englishHtml, "Copy evidence summary", "Rendered evidence summary cards should include explicit copy buttons.");
        RequireContains(englishHtml, "Narrative spine summary", "Rendered HTML should include a narrative-spine summary.");
        RequireContains(englishHtml, "Copy narrative spine", "Rendered HTML should include a narrative-spine copy button.");
        ...
        RequireContains(englishHtml, "process-tree-path", "Rendered process tree should keep image/path hints compact.");
        RequireContains(englishHtml, "depth 1", "Rendered process tree should expose depth badges for readability.");
        ...
        RequireContains(englishHtml, "Print/no-JS fallback", "Rendered HTML should include a print/no-JS fallback note.");
        RequireContains(englishHtml, "No JavaScript required for report navigation", "Rendered HTML should explain no-JS navigation.");
        ...
        RequireContains(chineseHtml, "打印 / 无 JS 兜底", "Chinese HTML should localize no-JS fallback.");
