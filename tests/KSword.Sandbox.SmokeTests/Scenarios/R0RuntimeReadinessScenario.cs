...
internal sealed class R0RuntimeReadinessScenario : ISmokeTestScenario
{
    ...
    public Task<SmokeTestResult> RunAsync(SmokeTestContext context, CancellationToken cancellationToken = default)
    {
        ...
        RequireContains(collectorDoc, "--diagnose", "Collector doc should document the live R0 diagnose mode.");
        RequireContains(collectorDoc, "r0collector.readinessDiagnostic", "Collector doc should document readiness diagnostic JSONL rows.");
        RequireContains(collectorDoc, "r0collector.readinessSummary", "Collector doc should document readiness summary JSONL rows.");
        RequireContains(collectorDoc, "severity", "Collector doc should document readiness severity fields.");
        RequireContains(collectorDoc, "readinessState", "Collector doc should document readiness state fields.");
        RequireContains(collectorDoc, "diagnosticCode", "Collector doc should document machine-readable diagnostic codes.");
        RequireContains(collectorDoc, "diagnosticStage", "Collector doc should document diagnostic stages.");
        RequireContains(collectorDoc, "missing_service", "Collector doc should distinguish missing driver services.");
        RequireContains(collectorDoc, "open_device_denied", "Collector doc should distinguish device ACL/elevation failures.");
        RequireContains(collectorDoc, "open_device_not_found", "Collector doc should distinguish missing device links.");
        RequireContains(collectorDoc, "abi_mismatch", "Collector doc should classify ABI mismatch as blocked.");
        RequireContains(collectorDoc, "read_timeout", "Collector doc should classify bounded read timeouts.");
        RequireContains(collectorDoc, "driver_no_events", "Collector doc should classify empty driver queues as degraded.");
