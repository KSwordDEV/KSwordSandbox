#include "Driver.h"

/*
 * Guards fixed-size IOCTL replies that have historically been consumed by
 * user-mode collectors.
 * Inputs : public reply layouts from KSwordSandboxDriverIoctl.h.
 * Logic  : C_ASSERT pins the v1.0 GET_HEALTH/GET_STATUS sizes, producer-mask
 *          offsets, queue stress aliases, and event metadata offsets so
 *          additions can safely reuse reserved space without silently changing
 *          the METHOD_BUFFERED ABI.
 * Return : no runtime value; build fails if the layout drifts.
 */
C_ASSERT(KSWORD_SANDBOX_INTERFACE_VERSION == 0x00010000U);
C_ASSERT(KSWORD_SANDBOX_EVENT_HEADER_VERSION == KSWORD_SANDBOX_INTERFACE_VERSION);
C_ASSERT(KSWORD_SANDBOX_EVENT_SCHEMA_VERSION == KSWORD_SANDBOX_INTERFACE_VERSION);
C_ASSERT(KSWORD_SANDBOX_ABI_VERSION_MAJOR == 1U);
C_ASSERT(KSWORD_SANDBOX_ABI_VERSION_MINOR == 0U);
C_ASSERT(sizeof(KSWORD_SANDBOX_HEALTH_REPLY) == 80U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_HEALTH_REPLY, ProducerEnableMask) == 44U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_HEALTH_REPLY, SupportedProducerMask) == 48U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_HEALTH_REPLY, ActiveProducerMask) == 52U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_HEALTH_REPLY, FailedProducerMask) == 56U);
C_ASSERT(sizeof(KSWORD_SANDBOX_STATUS_REPLY) == 120U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, LastNtStatus) == 36U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, ActiveProducerMask) == 40U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, FailedProducerMask) == 44U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, TotalEventsDropped) == 56U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, TotalEventsLost) == 56U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, QueueHighWatermark) == 24U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, TotalEventsBackpressured) == 88U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, ProducerDroppedMask) == 96U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, ProducerSuppressedMask) == 100U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, ProducerBackpressureMask) == 104U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, EffectiveProducerMask) == 108U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, LastFailureNtStatus) == 112U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_STATUS_REPLY, LastEnqueueFailureNtStatus) == 112U);
C_ASSERT(sizeof(KSWORD_SANDBOX_CAPABILITIES_REPLY) == 104U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_CAPABILITIES_REPLY, CapabilityFlags) == 16U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_CAPABILITIES_REPLY,
    EventHeaderVersion) == 32U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_CAPABILITIES_REPLY,
    ReadEventsReplyHeaderSize) == 44U);
C_ASSERT(sizeof(KSWORD_SANDBOX_SET_PRODUCER_ENABLE_MASK_REQUEST) == 48U);
C_ASSERT(sizeof(KSWORD_SANDBOX_SET_PRODUCER_ENABLE_MASK_REPLY) == 56U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_SET_PRODUCER_ENABLE_MASK_REPLY,
    EffectiveEnableMask) == 12U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_SET_PRODUCER_ENABLE_MASK_REPLY,
    SupportedProducerMask) == 16U);
C_ASSERT(sizeof(KSWORD_SANDBOX_READ_EVENTS_REQUEST) == 56U);
C_ASSERT(KSWORD_SANDBOX_READ_EVENTS_REPLY_HEADER_SIZE == 40U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_READ_EVENTS_REPLY, BytesWritten) == 16U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_READ_EVENTS_REPLY, EventsDropped) == 24U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_READ_EVENTS_REPLY, NextSequence) == 32U);
C_ASSERT((ULONG)FIELD_OFFSET(KSWORD_SANDBOX_READ_EVENTS_REPLY, Events) ==
    KSWORD_SANDBOX_READ_EVENTS_REPLY_HEADER_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_NETWORK_STATUS_REPLY) == 128U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_STATUS_REPLY, TodoMask) == 32U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_STATUS_REPLY, PayloadVersion) == 36U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_NETWORK_STATUS_REPLY,
    LastDegradeReason) == 40U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_NETWORK_STATUS_REPLY,
    RegisterNtStatus) == 48U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_STATUS_REPLY, ClassifyCount) == 56U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_STATUS_REPLY, EventCount) == 64U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_NETWORK_STATUS_REPLY,
    QueueFailureCount) == 72U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_NETWORK_STATUS_REPLY,
    LastClassifyLayerId) == 88U);
C_ASSERT(sizeof(KSWORD_SANDBOX_EVENT_HEADER) == 104U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, Sequence) == 16U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, ProcessId) == 32U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, ThreadId) == 40U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, ParentProcessId) == 56U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, LostEvents) == 64U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, BackpressureEvents) == 72U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, Operation) == 80U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, Status) == 84U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, ProducerId) == 88U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, ProducerMetadataFlags) == 92U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_EVENT_HEADER, TimestampSystemTime) == 96U);

/*
 * Verifies that every public producer payload fits the READ_EVENTS contract.
 * Inputs : compile-time structure definitions from KSwordSandboxDriverIoctl.h.
 * Logic  : C_ASSERT fails the WDK build if a future field expansion exceeds the
 *          fixed event payload capacity consumed by user-mode R0Collector.
 * Return : no runtime value; this translation unit exists for build-time ABI
 *          enforcement only.
 */
C_ASSERT(sizeof(KSWORD_SANDBOX_DRIVER_LOAD_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_FILE_EVENT_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_PROCESS_EVENT_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_IMAGE_EVENT_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_REGISTRY_EVENT_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);
C_ASSERT(sizeof(KSWORD_SANDBOX_NETWORK_EVENT_PAYLOAD) <=
    KSWORD_SANDBOX_EVENT_MAX_PAYLOAD_SIZE);

C_ASSERT(KSWORD_SANDBOX_PROCESS_EVENT_VERSION == 0x00010000U);
C_ASSERT(KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_VERSION == 0x00010001U);
C_ASSERT(KSWORD_SANDBOX_IMAGE_EVENT_VERSION == 0x00010000U);
C_ASSERT(KSWORD_SANDBOX_FILE_EVENT_VERSION == 0x00010000U);
C_ASSERT(KSWORD_SANDBOX_REGISTRY_EVENT_VERSION == 0x00010000U);
C_ASSERT(KSWORD_SANDBOX_NETWORK_EVENT_VERSION == 0x00010000U);
C_ASSERT(KswSandboxFileOperationSetSecurity == 9U);
C_ASSERT(KswSandboxRegistryOperationSetKeySecurity == 7U);

C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_DRIVER_LOAD_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_DRIVER_LOAD_PAYLOAD, Size) == 4U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_FILE_EVENT_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_FILE_EVENT_PAYLOAD, Size) == 4U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_PROCESS_EVENT_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_PROCESS_EVENT_PAYLOAD, Size) == 4U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT,
    Version) == 0U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT,
    Size) == 4U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT,
    Operation) == 8U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT,
    ObjectType) == 72U);
C_ASSERT(FIELD_OFFSET(
    KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT,
    TargetThreadId) == 80U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_IMAGE_EVENT_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_IMAGE_EVENT_PAYLOAD, Size) == 4U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_REGISTRY_EVENT_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_REGISTRY_EVENT_PAYLOAD, Size) == 4U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_EVENT_PAYLOAD, Version) == 0U);
C_ASSERT(FIELD_OFFSET(KSWORD_SANDBOX_NETWORK_EVENT_PAYLOAD, Size) == 4U);

C_ASSERT(sizeof(KSWORD_SANDBOX_DRIVER_LOAD_PAYLOAD) == 48U);
C_ASSERT(sizeof(KSWORD_SANDBOX_FILE_EVENT_PAYLOAD) == 128U);
C_ASSERT(sizeof(KSWORD_SANDBOX_PROCESS_EVENT_PAYLOAD) == 128U);
C_ASSERT(sizeof(KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT) == 128U);
C_ASSERT(sizeof(KSWORD_SANDBOX_IMAGE_EVENT_PAYLOAD) == 128U);
C_ASSERT(sizeof(KSWORD_SANDBOX_REGISTRY_EVENT_PAYLOAD) == 128U);
C_ASSERT(sizeof(KSWORD_SANDBOX_NETWORK_EVENT_PAYLOAD) == 112U);
