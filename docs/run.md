...
```powershell
.\install.ps1 -InstallEntrypoint CreateOrPreparePath -PromptPassword   # 首次或配置变化时运行
...
- **已配置环境：** 已有 `%ProgramData%\KSwordSandbox\install-state.json`、仓库外
  `sandbox.local.json`、guest password secret、VM/checkpoint profile 和 staged
  guest payload。直接跑只读检查后启动 WebUI。
- **恢复已有 checkpoint/snapshot：** VM 和 clean snapshot 已存在；先用
  `install.ps1 -Mode Change -UpdateHyperVConfig` 记录 VM/checkpoint，准备 payload，
  再由操作者在隔离 lab 中显式恢复 snapshot。恢复 VM 是 mutation，不是 `run.ps1`
  的默认动作。
- **创建/准备新环境：** 先完成 Windows Hyper-V host 兼容性、BIOS/UEFI Intel VT-x /
  AMD-V、Hyper-V PowerShell module、管理员 PowerShell、Windows guest、
  `SandboxUser`、Guest Service Interface、PowerShell Direct 和 clean checkpoint。
...
这些都是 release-safe 入口：不启动、不停止、不还原 VM。只有操作者在隔离 lab 中显式运行
...
`operatorModeMatrix` 必须与安装器实现一致；该检查不会执行安装模式或 Hyper-V mutation。
...
Plan/WebUI。Intel VT-x / AMD-V 是单独的 Hyper-V live 前置条件。
...
Live start 阶段会在 PowerShell Direct 就绪后，把 Guest Agent 作为已登录用户的
`InteractiveToken` Scheduled Task 启动，而不是只在远程 PowerShell 会话里启动。这样
Windows GUI 样本应显示在 Hyper-V basic console 桌面上；任务会以隐藏窗口启动
PowerShell wrapper，wrapper 再隐藏启动 `KSword.Sandbox.Agent.exe`，只保留样本窗口可见。
同时 Agent stdout/stderr、PID 和 exit marker 仍写入 job output 目录供 collector 等待与回收。
