...
- `GuestPayloadFreshnessReasons`: why the prepared Guest Agent/R0Collector
  ...
  repair command.
- `VmProfile`: confirms the operator-local VM name, checkpoint, guest working
  directory, runtime root, guest payload root, and driver path that a future
  explicit `-Live` run would use. Treat placeholder/example values as a
  configuration gap, not a packaging problem.
- `VirusTotalMissingKeyBehavior`: confirms that absent or failing optional
  ...
  logs. This optional VirusTotal API key is unrelated to Intel VT-x / AMD-V
  hardware virtualization, which is a Hyper-V host prerequisite.
