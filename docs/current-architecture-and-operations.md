...
2. Host 生成 job、SHA-256/静态分析事件（含 `static.pe.resource`）、
   PlanOnly/DryRun 或 Live Hyper-V runbook。
...
4. Guest Agent 采集 guest 行为，R0Collector 可在 mock 或 real R0 模式输出 `driver-events.jsonl`；
   ...
   counters、degrade reason 和 readiness state 作为采集质量证据。
...
已记录的本地 live evidence 见 `docs/webui-real-r0-e2e.md`。这是历史证据，不是本轮 fresh
validation；release notes 若要声明“本候选已生成真实 Notepad 5s/live 报告”，必须由
release manager 在准备好的 lab host 重新运行并记录 job id。该证据不改变仓库策略：样本、
...
Live API/WebUI E2E 会修改配置的 VM；必须先通过 readiness，并只在 lab host 上显式运行：
