...
2. Host 生成 job、SHA-256/静态分析摘要、PlanOnly/DryRun 或 Live Hyper-V runbook。
...
5. WebUI 通过后台 runbook state、progress snapshot 和 live raw-event endpoint 展示执行进度。
...
7. Host 导入 `events.json`、`driver-events.jsonl`、PCAP/artifact manifest 等 evidence，重跑规则并生成
   `report.json`、`report.html`、`report.zh.html`、`report.en.html` 和 artifact index。

已记录的本地 live evidence 见 `docs/webui-real-r0-e2e.md`。该证据不改变仓库策略：样本、VM 文件、
payload、job 目录、reports、PCAP、dump、build output、签名 driver、证书和 secrets 都不得提交。
...
- `src/KSword.Sandbox.Abstractions/`：共享 record 和 contract，覆盖 config、submission、sample
  ...
  host/guest 副作用。
- `src/KSword.Sandbox.Core/`：宿主侧业务逻辑。负责加载 config/rules、hash 与静态分析样本、构建
...
- `src/KSword.Sandbox.Web/`：ASP.NET WebUI/API。负责 dashboard、本地 executable upload/scan、job
  planning、后台 runbook start、progress/live-event endpoints、受保护 artifact download，以及可选
  VirusTotal hash lookup settings。
...
从 elevated shell 运行 live Hyper-V 分析：
...
如果 elevated process 看不到 `KSWORDBOX_GUEST_PASSWORD`，通过以下方式设置：
...
第一个命令是 dry-run；第二个命令会修改配置的 VM，必须在 readiness 通过后执行。
...
4. 运行 `./install.ps1 -Mode Install -PromptPassword`，把 local config 和 guest credential state 存在仓库外。
...
6. WebUI 启动时可让 `./run.ps1` best-effort 准备 guest payload；live run 前也可显式运行：
...
文档任务不要 build、不要 sign、不要 stage generated files、不要提交 runtime artifacts、不要 push。
