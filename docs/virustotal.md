...
Web 后端集成 VirusTotal 作为可选信誉富化（reputation enrichment）。它只用任务样本（job sample）的 SHA-256 调用官方
...
- Web 后端绝不把 key 写入磁盘（never writes API keys to disk）：不得写入仓库文件、runtime settings、report、event、
  cache entry 或 log。
- 缺少 key 时保持低噪音：端点返回 `status=not_configured`，不写任务消息、富化事件（enrichment events）
...
- `GET /api/jobs/{jobId}/virustotal` 默认只用于界面展示（UI display），不落盘。
- 只有显式 `?persist=true` 或 `POST /api/jobs/{jobId}/enrichments/virustotal` 才尝试保存富化结果（enrichment）。
...
- `not_configured`、`rate_limited`、`authentication_failed`、`timeout`、传输失败（transport failure）、解析失败（parse failure）只返回给
...
规则事件使用扁平字段和紧凑事件名（compact event name）`vt.lookup`，避免依赖嵌套 JSON 谓词。

## 缓存行为（cache behavior）

缓存是每个 Web host process 内存级（in-memory）的，key 为规范化 SHA-256 加非 secret API-key
指纹（fingerprint）。它会合并同一 hash 的并发查询（lookup），避免实时监控页刷新循环反复请求 VirusTotal。
...
- `verdict`：已收录报告（found report）的信誉判定（reputation verdict，`malicious`、`suspicious`、`clean`、`unknown`），
...
- `officialFileObject`：从官方 file object 中提取的操作者安全（operator-safe）元数据，包括 `id`、`type`、`md5`、
...
- `detectionPermalink`：直接进入检测页的 VirusTotal GUI URL，实时监控页（Live monitor）优先展示它作为主按钮。
...
UI 文案应明确“hash-only，不上传样本”。实时监控页应展示官方引擎统计、最后分析时间、信誉/社区
分数（reputation/community score）、可点击 VirusTotal 检测页链接和缓存状态。缺 key、not-found、rate-limit、auth failure、timeout 或其他失败
状态应安静展示在信誉卡片/健康区（reputation card / health area），不要作为样本恶意行为高亮，也不要默认写 job log。上传或选择样本后，
VT 卡片可以与真实 runbook progress、raw events 和 artifact index/download 卡片并列展示，但 VT 状态只是外部信誉补充，
...
富化端点（enrichment endpoint）执行同样的 hash-only lookup；只有官方已收录 `status=found` 时才写出单个提供方事件（provider event）。
面向规则的
