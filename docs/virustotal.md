...
## API Key 配置（process-only wording）
...
- Web 后端 never writes API keys to disk：不得写入仓库文件、config、runtime settings、report、event、cache entry 或 log。
...
- 在 job log / behavior log 中记录 API key、原始错误详情或样本内容。

## 静默状态 taxonomy（quiet states）

VT 是外部信誉/健康信息，不等于样本行为。默认只展示，不落盘：

- `GET /api/jobs/{jobId}/virustotal`：display-only。
...
只有 `status=found` 且调用方显式要求持久化时，才写 provider event。quiet states 不写 job log / behavior log，不附加 MITRE ATT&CK 映射，不进入主要恶意行为列表。
