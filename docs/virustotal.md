...
enrichment endpoint 执行同样的 hash-only lookup，然后写出单个 provider event，rule-facing type 为
`enrichment.virustotal.lookup`，event data 中 compact name 为 `vt.lookup`。扁平字段包括：
...
Response 包含 `canPersistEnrichmentEvent` 和 `persistedToEnrichmentEvents`，调用方可据此解释为什么某些
quiet states 没有落盘。
