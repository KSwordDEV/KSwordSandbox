...
- `screenshots/**`: `kind=Screenshot`, `evidenceRole=screenshot`,
  `collectionName=screenshots`；phase 从 `before-start`、`after-start`、`after-run` 等文件名推断。
- `memory-dumps/**`: `kind=MemoryDump`, `evidenceRole=memory-dump`,
  `collectionName=memory-dumps`, MIME type `application/vnd.microsoft.minidump`。
...
`phase` / `capturePhase`、`processRole`、`rootProcessId`、root `treeLineage`，以及用于报告文案的
...
（例如 `sourceHashStatus=failed`），但不会让整次运行失败。Collection metadata 保留 `lastReason` 和最后一次
...
- Process snapshots 在 `before-start`、`after-start` 和 `after-run` 为每个 sampled visible process 写出
  ...
  root-relative `treeDepth` / `treeLineage`。Tracked root/tree/new processes 如果在后续 snapshot 中消失，会以
...
`sha256`，以及样本进程身份已知时的 `processRole`、`rootProcessId`、`processId` 和 root `treeLineage`。Captured 与 skipped
rows 都包含 `zhMessage` / `zhHint`。Skipped screenshots 保留 `reason`、`diagnosticStage`、可选 `exceptionType` 和可选
`win32Error`，但不让 run 失败。未请求截图时，Guest 写出一个 `screenshot.disabled` event，字段包括
...
`dumpType`。Skipped attempts 保留 `reason`、`diagnosticStage`、可选 `exceptionType` 和可选 `win32Error`；captured/skipped/disabled
