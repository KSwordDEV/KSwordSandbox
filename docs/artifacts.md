...
`processRole`、`rootProcessId`、root `treeLineage`，以及用于报告文案的
...
`skippedReasonCounts` / `skippedReasonCountsJson`、`copiedHashComputedCount`、`copiedHashFailedCount`、
`sourceHashComputedCount` 和 `sourceHashFailedCount`。Collection metadata 保留 `lastReason` 和最后一次
...
- 每次 root sweep 写出 `process.tree.summary`，包含 `rootProcessId`、`rootVisible`、`visibleProcessCount`、
  ...
  metadata，以及用于 report process-tree rendering 的 `summaryEvent=true`。
...
`sha256`、`artifactIntegrityState=verified`，以及样本进程身份已知时的 `processRole`、`rootProcessId`、`processId` 和 root `treeLineage`。Captured 与 skipped
rows 都包含 `zhMessage` / `zhHint`。Skipped screenshots 保留 `reason`、`diagnosticStage`、可选 `exceptionType` 和可选
`win32Error`，并补充稳定的 `reasonCode` / `reasonCategory` / `zhReason`，但不让 run 失败。未请求截图时，Guest 写出一个 `screenshot.disabled` event，字段包括
...
字段），以及 `memoryDumpCoverageState`、`rootProcessCoverageState` 和 `childProcessCoverageState`，供报告卡片使用。
...
`rootAncestorProcessId` 和 `isDirectChildOfRoot`。Skipped attempts 保留 `reason`、稳定
`reasonCode` / `reasonCategory`、`diagnosticStage`、可选 `exceptionType` 和可选 `win32Error`；captured/skipped/disabled
...
`memory_dump.sweep` event 是 summary（`summaryEvent=true`），不计入 captured/skipped/failed collection status。
...
`reasonCounts` / `reasonCountsJson`，以及可用时的 first/last/largest artifact selectors。
