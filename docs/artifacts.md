...
`importMode=host-artifact-index` 和 `behaviorCounted=false`。这些是证据链/下载元数据（evidence-chain / download metadata），规则或 UI
...
`descendantDumpOptInMetadataVersion`，使 direct child 与 deeper descendant 采集意图无需从计数反推。
