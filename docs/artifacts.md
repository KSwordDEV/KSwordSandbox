...
生成的证据产物（artifact）都是运行期证据（runtime evidence）。不要提交 guest 输出目录、`artifacts/manifest.json`、
`artifact-index.json`、截图（screenshots）、内存转储（memory dumps）、抓包（packet captures）、掉落文件（dropped files）、报告、样本、
payload 二进制或 VM 输出。

高敏证据默认需要显式启用（opt-in）。稳定的 guest 输出根目录是传给 Guest Agent 的 `--out` 目录。本文中的
...
- `artifacts/manifest.json` 始终作为安全证据索引写出；当所有敏感采集通道（collection lane）都禁用时，它可以不包含文件 artifact。
  换句话说，Guest 会尽最大努力始终写出 `manifest.json`，作为 guest 侧证据索引（guest artifact index）；
  operator contract: `manifest.json` is always written as best-effort manifest output.
...
这些 optional artifact 事件和 collection summaries 使用同一组空值友好的 artifact 列：
`artifactRelativePath`、`sizeBytes`、`sha256`、`artifactRelativePathStatus`、`sizeBytesStatus`、
`sha256Status`、`artifactIntegrityState`、`rootProcessIdStatus` 和 `treeLineageStatus`。没有实际文件时
path/hash/size 为空，status 字段说明是 `disabled`、`skipped`、`failed`、`not-created` 还是 summary-only。

## Host 发现契约 / Host discovery contract

宿主侧发现（Host-side discovery）只把收集到的 guest 输出根目录作为链接根目录。`GuestOutputLocator.EnumerateArtifacts` 和
...
Descriptor 暴露 `relativePath`、URL 段编码的（URL-segment encoded）`safeLink` 和 `importPath` 作为浏览器/下载 selector。
`fullPath`、`guestPath`、`indexRoot` 和事件源路径（event source paths）只作为元数据或仅复制诊断（copy-only diagnostics）；它们不能直接写入
...
Host 写出 `artifact-index.json` 作为可下载证据索引（artifact index）。每个可下载证据条目（downloadable artifact entry）都带 host-computed
...
WebUI 下载体验（download UX）还使用这些 host descriptor 展示/安全元数据（presentation/safety metadata）：`previewLabel`、`previewLabelZh`、
...
Host 会用索引中实际存在的相对路径覆盖 descriptor metadata 里的 selector 字段；guest manifest 或 event 中原始的
absolute/traversal selector 只保留在 guestManifest*/rejection diagnostics，不会覆盖 `downloadSelector`、
`safeRelativeSelector`、`downloadSafeLink` 或 `reportRelativeHref`。

重复证据按 host-computed SHA-256 加字节大小分组。Index metadata 记录 `duplicateGroupKey`、
...
相对路径（relative path）是主项（primary）；后续 entry 仍可下载，但标记为 duplicate，方便 UI/report 折叠或标注重复的 dropped files、
...
如果 guest manifest 声明 unsafe、absolute、traversal、missing 或其他不可下载证据引用（non-downloadable artifact reference），Host 不会生成链接。
...
`artifactRejectionReasons`、`artifactRejectionReasonCountsJson`、machine-readable `artifactRejectionsJson`、
`lastRejectedArtifactReasonZh` 和 `zhRejectionHint`。这样既能保留操作者诊断，
...
`sensitiveCollectionReason`、`sensitiveCollectionReasonZh`、`downloadSelectorPolicy`、`safeSelectorFields`、
`zhStatus`、`zhReason` 和 `zhHint`。这些字段汇总
...
`downloadAvailable`、`downloadResolutionState`、`sensitiveArtifactReason`、`hostImportRowPurpose=download-metadata-and-evidence-chain`、
`importMode=host-artifact-index`、`behaviorCounted=false`、`metadataBehaviorCounted=false` 和
`metadataNonbehavior=true`。这些是证据链/下载元数据（evidence-chain / download metadata），规则或 UI
行为计数（behavior counts）不应把它们当作样本行为。

同一阶段还会写出 `artifact.import_summary`，汇总 `artifactCount`、`downloadableArtifactCount`、
`sensitiveArtifactCount`、`importedSensitiveArtifactCount`、`downloadableSensitiveArtifactCount`、
`sensitiveArtifactReasons`、`duplicateArtifactCount`、`duplicateGroupCount`、`rejectedArtifactCount`、
`downloadPolicy` 和 `rootPathPolicy`，并标记 `behaviorCounted=false` / `nonbehavior=true`。
如果导入的 `events.json` 位于 job result 目录外，Host 会把其相邻 guest output root 下已经复制成功的
`artifact.dropped_file.copied` 文件 materialize 到 `guest/{jobId}/artifacts/dropped-files`，并在 summary 中记录
`droppedFileMaterializedCount`、`droppedFileMaterializationExistingCount`、
`droppedFileMaterializationSkippedCount`、`droppedFileMaterializationReasonCountsJson` 和
`droppedFileMaterializationPolicy`；source 必须仍在 import root 下，绝对/穿越 selector 不会被复制。
存在 manifest 拒绝时，Host 会额外写出 `artifact.import_rejected` rows，把 `rejectedArtifactCount`、
`artifactRejectionReasons`、`artifactRejectionReasonCountsJson`、`lastRejectedArtifact*` 和
`artifactRejectionsJson` 放进报告状态；这些 rows 是导入诊断，
不参与样本行为计数。
...
复制成功的掉落文件 descriptor 始终带 `artifactRelativePath`、`relativePath`、
`collectionName=dropped-files`、`evidenceRole=dropped-file` 和 `captureState=captured`。已复制与已跳过的复制尝试（copied/skipped copy-attempt）
...
诊断形状。File-diff candidate rows 也保留同一列族：`artifactRelativePath`（空值配合
`artifactRelativePathStatus=not-created-by-file-diff`）、`sizeBytes` / `sha256` 以及
`sizeBytesStatus` / `sha256Status`，便于 copied、skipped 和候选 rows 用同一表格列展示。

跳过的复制尝试（skipped copy attempts）是非致命（nonfatal）的，`reason` 使用紧凑代码（compact code），例如 `sourcePathMissing`、`sourcePathInvalid`、
...
source/hash 诊断，使没有已复制文件（copied files）的运行也可诊断。未请求 dropped-file collection 时，Guest 写出
...
- 进程快照（Process snapshots）在 `before-start`、`after-start` 和 `after-run` 为每个采样到的可见进程（sampled visible process）写出
...
截图需要显式启用（opt-in），因为它可能包含与样本无关的桌面内容。截图是尽力采集（best-effort）的 BMP 文件，按配置的 `before,during,after` 阶段采集，
...
rows 都包含 `artifactRelativePath`、`sizeBytes`、`sha256` 及其 status 字段；skipped/disabled rows 使用空值配合
`artifactRelativePathStatus`、`sizeBytesStatus` 和 `sha256Status` 说明未产出原因。Captured 与 skipped rows 也都包含
`behaviorCounted=false` / `nonbehavior=true` / `notSampleBehavior=true` 和 `zhMessage` / `zhHint`。Skipped screenshots 保留 `reason`、`diagnosticStage`、可选 `exceptionType` 和可选
...
内存转储（Memory dump）默认关闭，必须通过 `--memory-dump` 或 `--memory-dumps` 显式启用。当前实现会在 `after-start` 为启动的样本进程
捕获一个 `MiniDumpNormal`，随后在 `after-run` process-tree sweep 中为仍可见的 child processes 尝试捕获 dump。
Dump attempts 写入 `memory-dumps/*.dmp`。

转储文件可能包含凭据或文档片段；Host 策略只应在确实需要该证据的 job 中启用。捕获失败（capture failures）会写出
...
`descendantCoverageCompleteness`，供报告卡片使用。显式 opt-in 范围还会写入
`descendantDumpOptInScope=root-plus-direct-children-and-deeper-descendants`、
`directChildProcessDumpEnabled`、`deeperDescendantProcessDumpEnabled` 和
`descendantDumpOptInMetadataVersion`，并补充 `descendantDumpOptInMode`、
`descendantDumpOptInOption`、`memoryDumpOptInApplied`、`rootDumpOptInApplied`、
`descendantDumpOptInApplied`、`directChildDumpOptInApplied` 和
`deeperDescendantDumpOptInApplied`，使 direct child 与 deeper descendant 采集意图无需从计数反推。
...
rows 均包含 `artifactRelativePath`、`sizeBytes`、`sha256` 及其 status 字段，以及 `zhMessage` / `zhHint`。Captured/skipped child targets also carry
...
每个 collection 记录 `enabled`、`implemented`、`status`、`reason`、`relativePath`、`safeLink` 和 `importPath`。禁用通道（disabled lanes）
显式存在，host importer 因而能区分“未请求（not requested）”与“已请求但不可用（requested but unavailable）”。Collection metadata 还记录 normalized
...
仍可按 `kind=PacketCapture` 导入和报告。Packet-capture started/stopped/skipped/failed/captured rows 使用同一 artifact 列族：
`artifactRelativePath`（pending/expected rows 指向预期 PCAPNG，disabled/skipped 无 session 时为空）、`sizeBytes`、`sha256`、
`artifactRelativePathStatus`、`sizeBytesStatus` 和 `sha256Status`。Captured/protocol-summary rows 同时写入
`pcapng*` 与 `etl*` integrity aliases，便于区分最终 PCAPNG 和诊断 ETL。
...
Guest collection summaries 还会暴露 `collectionSummaryVersion=artifact-collection-summary-v2`,
...
`largestArtifactSelector` selector 三元组（relative path、safe link、size、sha256）。
每个 summary metadata row 也保留 primary artifact 列族：有文件时 `artifactRelativePath` /
`sizeBytes` / `sha256` 指向按 relative path 排序的首个 artifact，status 为
`captured`/`computed`；没有文件时这些值为空，并用 `artifactRelativePathStatus`、
`sizeBytesStatus`、`sha256Status` 标记 `disabled`、`skipped`、`failed`、`not-created`
或 `not-implemented`。
...
每个文件 descriptor 还携带 `evidenceRole`、`capturePhase`、`captureState`、`guestPath`、`importPath` 和
...
WebUI 下载 selector 有意保持冗余且相对：`relativePath`、`safeLink` 和 `importPath` 都指向 job output root 下方。受保护的
下载端点（guarded download endpoint）在 URL 解码后接受这些 selector，但拒绝 empty、absolute、traversal、missing 或 unindexed paths。
...
`--packet-capture`、`--pcap` 和 `--network-capture` 是显式启用开关（opt-in switches）。它们在样本运行前启动 Windows `pktmon`，运行后停止，
...
启动/停止/转换命令失败（start/stop/convert command failures）会保留 `reason`、命令退出/超时详情和 `commandMessage`；失败的 `pktmon start` 记为
`packet_capture.failed`，不可用或未启动场景（unavailable/not-started cases）保持 `packet_capture.skipped`。未请求抓包时，Guest 写出一个
...
对导入/报告（import/report）来说，已有抓包文件（packet captures）是普通 artifacts：
