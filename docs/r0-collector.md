...
- Network correlation/versioning is explicit: the public ABI now names
  `KSWORD_SANDBOX_NETWORK_CORRELATION_CONTRACT_VERSION`,
  `KSWORD_SANDBOX_NETWORK_FLOW_KEY_VERSION`, and
  `KSWORD_SANDBOX_NETWORK_PROTOCOL_BOUNDARY_VERSION`.  JSONL rows expose these
  as `networkCorrelationContractVersion`, `flowKeyVersion`, and
  `protocolBoundaryVersion` so join semantics can evolve without claiming raw
  kernel payload parsing.
...
- 支持 `--abi-self-check` / `--contract-self-check`，用于 no-device ABI 和 event-quality
  ...
  `kernelBackpressurePolicy`、`queueLossEvidence`、
  `driverEventPayloadVersionPolicy` 和 `driverEventStressPolicy`。该行以及随后
  的 `r0collector.stopped` 行都显式设置 `behaviorCounted=false`、
  `nonbehavior=true`，随后在 `CreateFileW` / `DeviceIoControl` 前退出。
...
- `EventParser.*`：public driver ABI decoding 与 typed payload JSON mapping。
  Live driver rows always preserve generic `payloadVersion`,
  `payloadVersionHex`, `payloadSchemaVersion`, `payloadVersionStatus`, and
  explicit non-stress fields (`stress=false`, `stressOrdinal=-1`,
  `stressCorpusRole=live-driver-event`) before family-specific payload parsing.
...
同一 capabilities/health/readiness/status 证据还输出 R0/ETW 能力合同字段：
`r0EtwCapabilityContractVersion`、`r0EtwCapabilityContractSource`、
`r0DirectObservationScope`、`etwFallbackRequiredScope` 和每个 lane 的
`*R0Direct` / `*EtwFallbackRequired` / `*ObservationSource`。当前直接由 R0
producer 覆盖的 lane 是 `processCreateExit`、`imageLoad`、`fileActivity`、
`registryActivity` 和 `networkActivity`（WFP/ALE endpoint metadata only）。
`handleAccess` 只有在 live `GET_CAPABILITIES` 广告
`ProcessHandleAccessDraft`、`SupportedProducerMask` 包含 `processHandleAccess`，
且 `GET_STATUS` 的 active/effective producer mask 同时包含该 bit 时才算
R0 direct；否则仍进入 ETW/audit fallback，不能从 process create/exit 行推断。
当 draft 行实际出现时，pre-operation 行提供 requested access，post-operation
成功行提供 `grantedAccess` 和最终 `Status`；thread object 行使用 thread-rights
decoder，不按 process rights 解读。
发布默认 mask 不启用 draft `processHandleAccess`，因为 Object Manager handle callback
在干净 Notepad 5s 中也可能产生大量全局噪声；需要句柄访问实证时，直接对
R0Collector 使用 `--enable-mask 0x7f` 或在未来 runbook 配置中显式打开该 bit。
`tokenPrivilegeAdjustment` 当前不由 R0 v1 producer 直接发出，必须由 ETW/audit
fallback 补齐，直到未来显式广告并激活 `TokenPrivilegeDraft` capability bit。
同一合同还显式输出不可由当前 R0 v1 证明的动作面：`threadLifecycleR0Direct=false`、
`remoteThreadCreationR0Direct=false`、`serviceControlR0Direct=false`、
`driverServiceLoadSemanticsR0Direct=false`、`fileContentHashR0Direct=false`、
`registryValueDataBytesR0Direct=false`、`dnsPayloadR0Direct=false`、
`httpPayloadR0Direct=false` 和 `tlsPayloadR0Direct=false`。文件
`IRP_MJ_SET_SECURITY` 与注册表 `RegNtPostSetKeySecurity` 只作为 R0 metadata
rows（路径/状态/security-information mask）输出，不包含 ACL 或 security
descriptor bytes。上述字段只说明 coverage/fallback 所属方，不是样本 verdict；
对应 owner 见 `r0FallbackOwners`。

同一批 control-plane rows 还输出 `r0Coverage*` readiness fields，供报告直接说明
已审计覆盖面和 known gaps。审计 taxonomy 固定为
`driver.load|process.lifecycle|image.load|file.activity|registry.activity|network.metadata|queue.loss.backpressure`。
`r0CoverageAvailableCategories` 来自 capabilities supported mask、status active/effective
mask、producer-mask effective mask 或 queue snapshot；`r0CoverageKnownGaps` 与
`r0CoverageFallbackOwners` 明确列出 process handle access、thread lifecycle/remote
thread creation、token privilege/object handles、service-control/driver-load semantics、
network raw-packet/DNS/HTTP/TLS payload、bounded path/command-line prefixes、registry
value data bytes、file content/hash、file/registry security descriptor bytes 和
user-mode stack 等需要 draft R0 payload、ETW/audit、PCAP/sidecar 或 guest artifact
补齐的缺口。中文提示为 `zhR0CoverageHint`；
机器逻辑应使用英文 `r0Coverage*` 字段。
...
`todoMaskNames` splits remaining network work into packet/stream layers, flow
contexts, filter conditions, protocol payloads, DNS/HTTP/TLS payload parsers,
and PCAP sidecar join readiness.  These are explicit limitations/readiness
gaps, not hidden kernel parser promises.
...
每条 drained driver row 都会附加 `r0CoverageCategory`、`r0CoverageState`、
`r0CoverageEvidenceSource=drained-driver-event-row`、`r0CoverageKnownGaps`、
`r0CoverageFallbackOwners` 和 `r0ReadinessContribution`。例如 `driver.process` 使用
`r0CoverageCategory=process.lifecycle`，`driver.network` 使用
`r0CoverageCategory=network.metadata` 与 `r0CoverageState=covered-metadata-only`。
这些字段让报告可以把“已覆盖的防御事件类别”和“仍需 fallback 的缺口”放在同一事件旁边，
而不是从 `eventType`、`operationName` 或 candidate booleans 间接推断。

Process-handle access 是 draft-only readiness path：当前 v1 driver 不应被报告为
已经提供 ObRegisterCallbacks 句柄访问遥测，除非 live capabilities 显式广告
`ProcessHandleAccessDraft`，或 `READ_EVENTS` 行携带
`KSWORD_SANDBOX_PROCESS_HANDLE_ACCESS_EVENT_PAYLOAD_V1_DRAFT` 的 draft
`Version`/`Size`/handle operation prefix。Collector 已准备好非侵入式解析该 future
payload：匹配时 top-level `eventType` 归一为 `process.access`，`data.payloadSchemaName`
为 `r0.process_handle_access`，并输出 actor/target/source/duplicate-target PID、
可选 target TID、`objectTypeName`、`accessMaskDecodeScope`、
`originalDesiredAccess`、`desiredAccess`、`grantedAccess` 的 hex/name/敏感位解码
（例如 process VM_WRITE/CREATE_THREAD/DUP_HANDLE/TERMINATE 或 thread SET_CONTEXT/
SET_TOKEN/IMPERSONATE）。中文提示字段会明确说明：
这些权限字段只来自显式 handle-access payload，不能从普通 process create/exit
payload 推断。噪声策略仍沿用 collector/self-noise 分类；access mask 或 target PID
本身不构成噪声，命中 collector PID、KSword infrastructure path 或 producer self-noise
flag 时才按 `--suppress-self-noise` 抑制。
...
WFP event naming is also normalized for user mode: `wfpEventFamily`,
`wfpEventName`, `wfpLayerSemanticName`, `wfpLayerDirection`,
`wfpLayerAddressFamily`, `wfpInspectionAction`, `kernelNetworkProducerScope`,
and `networkWfpEventNameFields` distinguish ALE connect/recv-accept IPv4/IPv6
events without requiring WDK constants in report consumers.
...
Live and synthetic `driver.network` rows now keep the same additive boundary
vocabulary: `protocolPayloadSource=none-r0-endpoint-metadata-only`,
`rawPacketPayloadAvailable=false`, `kernelPayloadParserEnabled=false`,
`pcapCorrelationStatus=required-unmatched-in-r0-row`, `pcapFlowKeyCandidate`,
`pcapCorrelationKey`, `pcapExpectedRecordTypes=pcap.flow|pcap.dns|pcap.http|pcap.tls`,
`pcapDnsDetailsAvailable=false`, `pcapHttpDetailsAvailable=false`,
...
不是协议解析 verdict。R0Collector 不从 ALE payload 中解析 raw packet bytes、DNS name、
HTTP Host/URI/method 或 TLS SNI/certificate；它只保留 endpoint/PID/layer/callout/filter 信息，让 Host import 可以把这些行与
...
- Guest Agent 的默认 sidecar 参数面向 MVP live 稳定性：`--max-events 1024`、
  `--max-read-batches 0`（不因固定批次上限提前停读）以及
  `--driver-event-sample-stride 16`。这会持续 drain R0 ring 以降低 backpressure，
  但只写出采样后的 JSONL 行；完整处理数量仍在 `driverRecordsProcessed`、
  `collectorSkippedEvents`、`lostCount`、`highWatermark` 和 producer mask 字段中可审计。
...
- `--stress-count <n>`: emit `n` contiguous synthetic `driver.file` rows with
  ...
  `semanticSelfCheckScenarios` / `semanticSelfCheckRows`. Additive fields such
  as `semanticRowKind`, `semanticRowCountedInStress`, and
  `semanticStressCountImpact` disambiguate counted stress rows from the 6
  semantic companion rows without changing row counts.
- `--inject-jsonl-noise`: in mock/stress mode, append a blank line, malformed
  JSON row, and valid `driver.network` row with an ignored extra top-level field.
  The valid row is marked with `noiseScope=synthetic-jsonl-noise`,
  `noiseKind=valid-extra-field-row`, `noiseDisposition`, and TLS/PCAP boundary
  fields. This proves the Host live/import path can tolerate partial JSONL
  without hiding valid rows.
...
- `r0collector.readinessDiagnostic` with `diagnosticStage=abiNegotiation`. Codes
  ...
  ABI rows include stable expected-vs-actual fields such as
  `expectedAbiVersionMajor`, `driverAbiVersionMajor`,
  `expectedEventHeaderVersion`, `driverEventHeaderVersion`,
  `expectedReadEventsReplyHeaderSize`, `driverReadEventsReplyHeaderSize`,
  per-field match booleans, and `abiMismatchReasons`.
...
- Protocol parser boundary fields are explicit on synthetic network/noise rows:
  ...
  浏览器/sidecar 证据补齐。
  Live network rows emit the same boundary family plus `protocolPayloadSource`,
  `pcapCorrelationStatus`, `pcapFlowKeyCandidate`, `pcapCorrelationKey`,
...
- `abiSelfCheckDiagnosticsVersion`, `abiSelfCheckDiagnosticLevel`,
  `abiGuardDiagnosticSummary`, `abiGuardDriftAction`, `abiGuardFieldSet`,
...
- READ_EVENTS: `readEventsRequestSize`, `readEventsReplyHeaderSize`,
  `readEventsBytesWrittenOffset`, `readEventsEventsDroppedOffset`,
  `readEventsNextSequenceOffset`, and `readEventsEventsOffset`.
- Capabilities / producer-mask negotiation: `capabilitiesReplySize`,
  `capabilitiesCapabilityFlagsOffset`, `capabilitiesEventHeaderVersionOffset`,
  `capabilitiesReadEventsReplyHeaderSizeOffset`,
  `setProducerEnableMaskRequestSize`, `setProducerEnableMaskReplySize`,
  `setProducerEnableMaskReplyEffectiveMaskOffset`, and
  `setProducerEnableMaskReplySupportedMaskOffset`.
...
- Typed payload versions/sizes: `driverLoadPayloadVersion`,
  `processPayloadVersion`, `imagePayloadVersion`, `filePayloadVersion`,
  `registryPayloadVersion`, `networkPayloadVersion`, their `*Hex` aliases,
  their `*PayloadVersionOffset`/`*PayloadSizeOffset` prefix guards,
  `driverLoadPayloadSize`, `processPayloadSize`, `imagePayloadSize`,
  `filePayloadSize`, `registryPayloadSize`, `networkPayloadSize`,
  `eventMaxPayloadSize`, `payloadVersionPolicy`, and
  `producerPayloadVersionFieldSet`.
- Stable semantic/noise fields: `stableJsonlFields`,
  `typedPayloadSemanticFields`, `selfNoiseClassificationFields`,
  `stressBackpressureDiagnostics`, `driverEventStressFieldSet`,
  `driverEventStressPolicy`, `driverEventPayloadVersionPolicy`, and
  `queueLossEvidence`.
...
只帮助人工理解，不替代英文机器字段。Known v1 producer rows should also preserve
`payloadVersion=65536` / `payloadVersionHex=0x00010000`; mismatches are ABI
evidence and should not be counted as successful typed payload parsing.
...
- `data`: string-valued metadata compatible with the current host model.
  ...
  `payloadSchema` when the payload category is known. All live and synthetic
  driver rows also include generic payload-version aliases
  `payloadVersion`, `payloadVersionHex`, `payloadSchemaVersion`,
  `expectedPayloadVersion`, and `payloadVersionStatus`, plus stable stress
  aliases (`stress`, `stressOrdinal`, `stressCount`, `stressCorpusRole`) so
  mock/live JSONL remain comparable.
...
- ABI structure version evidence: capabilities and driver rows preserve
  ...
  `payloadSchema`; driver rows additionally preserve `payloadVersion`,
  `payloadVersionHex`, `payloadSchemaVersion`, `expectedPayloadVersion`,
  `payloadVersionStatus`, and `producerPayloadVersionFieldSet`.
...
- Attribution/self-noise evidence: mock and ABI self-check rows preserve
  ...
  `collectorNoisePolicy`, `behaviorCounted`, `nonbehavior`, and
...
- `StressJsonlNoiseEvidence`: expected noise/self-noise metadata fields for
  noisy stress imports, including `noise`, `noiseClass`, `noiseDecision`,
  `collectorNoise`, `collectorSelfNoise`, `selfNoise`, `selfProcess`,
  `behaviorCounted`, `nonbehavior`, and `sampleBehaviorCandidate`.
- `driverEventStressFieldSet`: live driver rows use explicit non-stress values
  (`stress=false`, `stressOrdinal=-1`, `stressCorpusRole=live-driver-event`);
  counted synthetic rows set `stress=true` and keep the same field names.
