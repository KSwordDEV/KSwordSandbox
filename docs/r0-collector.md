...
`httpPayloadR0Direct=false` 和 `tlsPayloadR0Direct=false`。这些字段只说明
coverage/fallback 所属方，不是样本 verdict；对应 owner 见 `r0FallbackOwners`。
...
value data bytes、file content/hash 和 user-mode stack 等需要 draft R0 payload、
ETW/audit、PCAP/sidecar 或 guest artifact 补齐的缺口。中文提示为 `zhR0CoverageHint`；
...
（例如 VM_WRITE、CREATE_THREAD、DUP_HANDLE、TERMINATE）。中文提示字段会明确说明：
