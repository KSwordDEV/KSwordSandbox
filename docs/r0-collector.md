...
mask、producer-mask effective mask 或 queue snapshot；`r0CoverageKnownGaps` 明确列出
process handle access、token privilege adjustment、network raw-packet/DNS/HTTP/TLS payload、
bounded path/command-line prefixes、registry value data bytes、file content/hash 等需要
draft R0 payload、ETW/audit、PCAP/sidecar 或 guest artifact 补齐的缺口。中文提示为
`zhR0CoverageHint`；机器逻辑应使用英文 `r0Coverage*` 字段。
...
`r0CoverageEvidenceSource=drained-driver-event-row`、`r0CoverageKnownGaps` 和
`r0ReadinessContribution`。例如 `driver.process` 使用
