...
5. Core live telemetry service 以 shared-read 容错方式读取 JSON/JSONL。
...
实时显示不是 verdict。它只是运行中可见性通道，帮助操作者确认进程、文件、注册表、图像、R0 和网络事件是否正在回收。风险评分、MITRE 映射和最终结论仍属于报告再生成路径。
...
`live.events.source_status` 是 Host 生成的 raw event。它使用固定时间戳 `1970-01-01T00:00:00Z` 与稳定排序，避免同一文件状态反复读取时 cursor 抖动。字段包括 `format`、`sourceFileName`、`eventCount`、`recordCount`、`blankLineCount`、`parseErrorCount`、`partialLineCount`、`sizeBytes`、`lastWriteTimeUtc` 和 `cursorStable=true`。该行仅用于诊断；最终报告归类仍使用导入后的 durable events。

## 浏览器 API 面 / API surface

动态监控页面面向浏览器提供以下入口：

- `GET /jobs/{jobId}/live-events`：独立 live raw event page。
- `GET /api/jobs/{jobId}/events/stream`：原始事件 Server-Sent Events (SSE)。
- `GET /api/jobs/{jobId}/events/live`：原始事件 JSON 轮询 fallback。
- `GET /api/jobs/{jobId}/progress/stream`：真实 runbook step 进度 SSE。
- `GET /api/jobs/{jobId}/runbook/progress`：进度快照轮询 fallback。
- `GET /api/jobs/{jobId}/runbook/background`：后台执行状态轮询 fallback。
...
## 轮询 fallback / Polling fallback

当 SSE 不可用或不适合当前环境时，轮询仍是 WebUI 与 QA probe 的必备 fallback。客户端应在以下情况回退到 `/api/jobs/{jobId}/events/live?offset=<lastOffset>&take=100` 或进度轮询 API：
