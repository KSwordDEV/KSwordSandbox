...
with no duplicate IDs in the cheap JSON audit. The current v29 release-handoff
HEAD does not add a new behavior-rule batch; the rule-set version remains the
v28 expansion, and the v28 high/medium rules retain the host, collection-health,
VirusTotal quiet-state, R0 health/self-noise, and sandbox plumbing process
guards checked by the lightweight guard audit. The v28 batch adds 10 bounded
