...
安装阶段只负责本机配置和只读诊断；它不会生成 `componentProgress`，但 release 打包/readiness 会把
安装侧缺口汇总到 `operator-remediation-zh`。操作者看到 Hyper-V、VM profile、guest payload、VT key、
runtime root 或 `RuntimePublishRoot` 缺口时，先运行：
...
然后按输出的 `RecommendedActions` 修复。不要用安装器补 fresh live evidence；没有实验室 `job id` 时，
发布说明仍必须写“本候选未刷新 fresh live evidence”。
