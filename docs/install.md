...
1. 新电脑或普通用户直接运行 `.\install.ps1`，按推荐安装向导回答常用设置：
   runtime root、VM/checkpoint、guest 用户、guest 密码、可选 R0 driver path、
...
3. 自动化/CI 才需要显式参数，例如
   ...
   `.\install.ps1 -Mode Change -UpdateHyperVConfig -VmName <VM> -CheckpointName <Snapshot>` 写入脚本化配置。
...
向导会询问 runtime root、已有 Hyper-V VM、已有 clean checkpoint、guest 用户、
...
如果还没有 VM，请先在 Hyper-V 中创建/导入 Windows guest，建立
`SandboxUser` 或等价本地账号，启用 Guest Service Interface，确认 PowerShell Direct
可用，并创建 clean checkpoint/snapshot（常用名 `Clean`）。然后重新运行 `.\install.ps1`
按提示选择/输入 VM 和 checkpoint。
