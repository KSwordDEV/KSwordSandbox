...
- `RestoreCleanCheckpoint` 是 rollback 语义：默认 `-PlanOnly`/safe diagnostics；
  ...
  `-AllowVmMutation`，并再加 `-Confirm` 或 `-Force`。
- `CreateOrPreparePath` 是 fresh new-computer 本机准备：只写仓库外目录、local config、
  secret 和可选 payload；它不创建 Hyper-V VM，也不创建 checkpoint。
...
如果 VM 已经漂移，实际恢复 checkpoint/snapshot 是明确的 lab mutation，只能由操作者在
隔离 host 上显式执行，例如：
...
适用于第一台电脑或全新实验室。先在 Hyper-V 中创建/导入 Windows guest，建立
