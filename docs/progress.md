...
本轮快照反映本地 v22+ 发布准备批次（基线 `77298d6 / v22`）的文档和源码能力：v21/v22 已补强 telemetry/product-readiness polish、
defensive behavior matrix、artifact/download metadata、network address-scope metadata、live progress freshness 和 runtime guardrail。当前没有新的 build 矩阵、真实样本报告、
fresh live evidence `job id` 或完整 `RuntimePublishRoot` handoff 证据；后续只有在记录新的 readiness/package 结果、
source package dry-run、完整 runtime handoff 或实验室 live `job id` 与报告路径后，才继续上调这些数字。
...
- Host 静态分析输出 `static.analysis.completed` 兼容摘要，并投影 `static.pe.*`、`static.string.*`、`static.packer.hint`、`static.yara.match`、`static.pe.resource` 等结构化事件供规则和报告消费；行为规则当前 550 条、重复 ID 组为 0。
