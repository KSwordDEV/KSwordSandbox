...
public static class SampleHasher
{
    ...
    /// existence, size, SHA-256, SHA-1, MD5, and CRC32; the method returns
    ...
    public static SampleIdentity Compute(string samplePath, long maxSampleBytes)
    {
        ...
        return new SampleIdentity
        {
            FileName = info.Name,
            FullPath = fullPath,
            Sha256 = HashFile(fullPath, SHA256.HashData),
            Sha1 = HashFile(fullPath, SHA1.HashData),
            Md5 = HashFile(fullPath, MD5.HashData),
            Crc32 = ComputeCrc32(fullPath),
            SizeBytes = info.Length
        };
    }

    /// <summary>
    /// Computes one cryptographic digest for a file.
    /// Inputs are a file path and hashing delegate, processing streams the file
    /// through the delegate, and the method returns lowercase hexadecimal text.
    /// </summary>
    private static string HashFile(string fullPath, Func<Stream, byte[]> hash)
    {
    ...
    /// <summary>
    /// Computes CRC32 for compatibility with common sandbox report fields.
    /// The input is a file path, processing streams bytes through a static
    /// lookup table, and the method returns uppercase eight-character hex.
    /// </summary>
    private static string ComputeCrc32(string fullPath)
    {
        var crc = 0xffffffffu;
        ...
        while ((read = stream.Read(buffer)) > 0)
        {
            ...
            {
                var tableIndex = (int)((crc ^ buffer[index]) & 0xff);
    ...
    /// <summary>
    /// Lookup table for the standard CRC32 polynomial.
    /// The field has no inputs, stores precomputed constants, and returns no
    /// value by itself; ComputeCrc32 indexes it while streaming file bytes.
    /// </summary>
